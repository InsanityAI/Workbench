...
do
    ...
    if _EXPERIMENTAL then
        OnInit.main("LIGUITimers", function(require)
            ...
            local TimerQueue = TimerQueue.create()
            ...
            function TriggerRegisterTimerEvent(whichTrigger, timeout, periodic)
                ...
                -- todo: what to do about an event it returns?
                -- what can trigger event even do?
            end
            ...
            function TriggerRegisterTimerExpireEvent(whichTrigger, t)
                if check(whichTrigger ~= nil, 'trigger cannot be nil') then return nil end
                if check(t ~= nil, 'timer cannot be nil') then return nil end
    ...
    if _EXPERIMENTAL then
        OnInit.main("LIGUITriggers", function(require)
            ...
            -- Events
            do
                ---@class AbstractTriggerEvent: FakedType
                ...
                EventRegistry.FilterUnit = defineDynamicEventTypeWithFilter(TriggerRegisterUnitEvent, 4,
                    unitEventResponseMap) -- actually overrides TriggerRegisterFilterUnitEvent but uses the no-filter one
