...
--[[——————————————————————————————————————————————————————
    Total Initialization version 6.0.1
    Created by: Bribe
...
do
    ...
    local function callUserFunc(libraryName, func)
        if not func then
            ---@cast libraryName Initializer.Callback
            func = libraryName
        else
            assert(type(libraryName) == 'string', 'Module name must be string! Provided: ' .. tostring(libraryName))
            -- Note: this function is called when OnInit inside OnInit occurs, at which point both OnInit calls are in the same file.
        ...
        assert(type(func) == 'function', 'Expected module function. Provided: ' .. tostring(func))
    ...
    local errorHandler = function(msg)
        return "|cFFFF0000OnInit error: " .. msg .. "|r"
    end

    ---@param func function
    ---@param ... unknown
    ---@return boolean success
    local tryCall = Debug.try or function(func, ...)
    ...
    local function createInit(name)
        ...
        return function(libraryName, userInitFunc, debugLineNum)
            tryCall(addUserFunc, name, libraryName, userInitFunc, debugLineNum)
        end
    end
    ...
    do
        ...
        local function __call(
            ...
            if userInitFunc or type(libraryNameOrInitFunc) == 'function' then
                success, errorMsg = pcall(addUserFunc,
                    'InitGlobals',
                    libraryNameOrInitFunc,
                    userInitFunc,
                    debugLineNum,
            ...
            elseif library then
                success, errorMsg = pcall(library.declare, library, libraryNameOrInitFunc) --API handler for OnInit "Custom initializer"
                if not success then
                    print("|cFFFF0000OnInit error: " .. errorMsg .. "|r")
                    return
                end
    ...
    if library then
        ...
        local function declareName(name, initialValue)
            ...
            assert(library.moduleValueMatrix[name] == nil, 'Module already declared! Module name: ' .. name)
            library.moduleValueMatrix[name] =
                initialValue and { true, n = 1 }
        end

        function library:create(name, userFunc)
            assert(type(userFunc) == 'function', 'Module function expected! Provided: ' .. tostring(userFunc))
        ...
        function processRequirement(
            ...
            assert(type(source) == 'table', 'Requirement source must be a table. Provided: ' .. tostring(source))
            assert(type(requirement) == 'string', 'Requirement must be a string! Provided: ' .. tostring(requirement))
