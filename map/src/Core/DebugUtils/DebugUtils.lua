...
*    Debug.traceback([startDepth:integer, endDepth:integer]) -> string
*        - Debug.traceback() without arguments returns the complete stack trace leading to the code position where this function is called. You need to manually print it.
*        - You can specify a startDepth (default: 0) and endDepth (default: 200) to instead return the stack trace of the function calls between startDepth and endDepth. Do this, if you want to call Debug.traceback from a wrapper.
...
*        - For instance, print(CreateUnit) will show "function: CreateUnit" on screen instead of "function: 0063A698". If you still need the second, use Debug.original.tostring or Debug.original.rawprint instead.
...
*        - Note that Debug.original.print will still show overridden behaviour (applying names from the name caching), because it internally relies on  (overridden) tostring. 
*          If you need print without name caching, use Debug.original.rawprint (which is not actually an original function, but reconstructed).
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------]]
    ...
    Debug = {
        ...
        ,   original = {
            ...
            ,   tostring = tostring
            ,   print = print --this IS the original print function, but it will still use the overridden tostring (applying names from the name caching) as per implementation of print in Lua 5.3. If you need print without name caching, use Debug.original.rawprint instead.
            ,   rawprint = print --Defined further below. Use this to print stuff using the original tostring, i.e. in the format <type>: <memoryAddress>. This is not actually an original function, but tries to resemble the original print behaviour, before it was affected by overridden tostring.
    ...
    Debug.data.sourceMap[1].firstLine = tonumber(codeLoc:match(":%d+"):sub(2,-1))

    local originalToString, originalPrint = Debug.original.tostring, Debug.original.print
    Debug.original.rawprint = function(...)
        local n = select('#', ...)
        local toprint = n > 0 and originalToString(select(1, ...)) or ''
        for i = 2, n do
            toprint = toprint .. '    ' .. originalToString(select(i, ...))
        end
        originalPrint(toprint)
    end
    ...
    function Debug.getLine(depth)
        depth = depth or 0
        local _, location = pcall(error, "", depth + 3)
        local line = location:match(":%d+") --extracts ":1000" from "war3map.lua:1000:..."
    ...
    function Debug.getLocalErrorMsg(errorMsg)
        local startPos, endPos = errorMsg:find(":%d*") --start and end position of line number. The part before that is the document, part after the error msg.
    ...
    ---@return string trace
    function Debug.traceback(startDepth, endDepth)
        ...
        for loopDepth = startDepth, endDepth do --get trace on different depth level
            ...
            if #tracePiece > 0 and lastTracePiece ~= tracePiece then --some trace pieces can be empty, but there can still be valid ones beyond that
                ...
                if settings.INCLUDE_DEBUGUTILS_INTO_TRACE or (loopDepth == startDepth) or currentFile ~= "DebugUtils:" then
                    trace = trace .. separator .. ((currentFile == lastFile) and tracePiece:match(":%d+"):sub(2,-1) or tracePiece:match("^.-:%d+"))
    ...
    function Debug.errorHandler(errorMsg, startDepth, returnErrorMsg_yn)
        startDepth = startDepth or 2 --default 2 to make this callable with the correct depth from within xpcall (which doesn't specify this param)
        ...
        if settings.SHOW_TRACE_ON_ERROR then
            toPrint = toPrint .. "\n|cff" .. settings.colors.error .. "Traceback (most recent call first):|r\n|cff" .. settings.colors.error .. Debug.traceback(startDepth,202) .. "|r"
        end
        --Also print entries from param log, if there are any.
        for location, loggedParams in next, paramLog do
            toPrint = toPrint .. "\n|cff" .. settings.colors.log .. "Logged at " .. Debug.getLocalErrorMsg(location) .. loggedParams .. "|r"
        ...
        return returnErrorMsg_yn and toPrint
    end
    ...
    function Debug.try(funcToExecute, ...)
        return select(2, xpcall(funcToExecute, Debug.errorHandler,...))
    end
    try = Debug.try --global accesible try-function for downwards compatibility to older versions.
    ...
    ---@return string errorMsg including stack trace
    function Debug.throwError(...)
        if not data.firstError then
            Debug.errorHandler(Debug.traceback(2,2) .. ": " .. concat(...), 3)
        end
    end
    ...
    function Debug.assert(condition, errorMsg, ...)
        if condition then
            ...
            Debug.errorHandler(Debug.traceback(2,2) .. ": " .. errorMsg, 3)
        end
    end
    ...
    function Debug.log(...)
        local _, location = pcall(error, "", 3)
    ...
    local function addNameToCache(parentTableName, key, object, parentTableDepth)
        ...
        if type(key) == 'string' and not string.find(key, "%s") then
    ...
    local function registerAllObjectsInTable(parentTable, parentTableName)
        ...
        for key, object in next, parentTable do --use native pairs even if a custom __pairs metamethod has been defined. We only want to add names for true subtables, not pseudo-ones. This should also prevent bugs with multi-parameter implementations of next,  like in MDTable.
    ...
    function Debug.registerNamesFrom(parentTable, parentTableName, depth)
        ...
        if depth > 1 then
            for _, object in next, parentTable do --use native pairs even if a custom __pairs metamethod has been defined. We only want to add names for true subtables, not pseudo-ones. This should also prevent bugs with multi-parameter implementations of next,  like in MDTable.
    ...
    if settings.USE_PRINT_CACHE then
        ...
        print = function(...)
            if not settings.USE_PRINT_CACHE then
                oldPrint(...)
        ...
        if settings.WARNING_FOR_NIL_GLOBALS then
            ...
            getmetatable(_G).__index = function(t, k) --we made sure that _G has a metatable further above.
                ...
                if data.printErrors_yn and (not globalWarningExclusions[k]) and ((not settings.EXCLUDE_BJ_GLOBALS_FROM_NIL_WARNINGS) or string.sub(tostring(k),1,3) ~= 'bj_') then --prevents intentionally nilled bj-variables from triggering the check within Blizzard.j-functions, like bj_cineFadeFinishTimer.
                    print("|cff" .. settings.colors.nilWarning .. "Trying to read nil global at " .. Debug.traceback(2,2) .. ": " .. tostring(k) .. "|r"
                        .. (settings.SHOW_TRACE_FOR_NIL_WARNINGS and "\n|cff" .. settings.colors.nilWarning .. "Traceback (most recent call first):|r\n|cff" .. settings.colors.nilWarning .. Debug.traceback(2,200) .. "|r" or ""))
                end
    ...
    if settings.USE_PRINT_CACHE then
        local originalInitGlobals = InitGlobals
